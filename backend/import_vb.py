"""One-off: register the Nintendo Virtual Boy romset (already extracted into
/roms/vb/) into the DB so the DB-driven library doesn't desync from the dropped
files. Mirrors the column set the rest of the library uses (see import_lynx.py).
Idempotent: skips files already in the roms table.

Covers are left empty (cover_status='none'); they're fetched later via the
cover-search UI / autofill (igdb/tgdb/libretro mappings were added for vb).
"""
...
files = sorted(p for p in rom_dir.iterdir() if p.suffix.lower() == ".vb")
...
print(f"vb import: added {added}, skipped {skipped} (already present), total files {len(files)}")

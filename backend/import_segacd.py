"""One-off: register the Sega CD romset (already dropped into /roms/segacd/) into
the DB so the DB-driven library doesn't desync from the dropped files. Mirrors the
column set the rest of the library uses (see import_vb.py). Idempotent: skips
files already in the roms table.

All 15 titles here are standalone .chd dumps (no .cue/.bin sidecars), so each is
a normal flat rom row like any other system — no extra_files/folder handling
needed (that path is for .cue+track sets, see roms.py upload_cd_folder).

Covers are left empty (cover_status='none'); fetched later via the cover-search
UI / autofill (igdb/tgdb/libretro mappings were added for segacd).
"""
...
files = sorted(p for p in rom_dir.iterdir() if p.suffix.lower() in (".chd", ".cue"))
...
print(f"segacd import: added {added}, skipped {skipped} (already present), total files {len(files)}")

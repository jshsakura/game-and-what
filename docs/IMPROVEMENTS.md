...
| # | 항목 | 성격 | 영향 | 수정 비용 |
|---|---|---|---|---|
| [A-1](#a-1-라이브-데모가-잘못된-시스템-정보를-표시-3건) | 데모가 잘못된 시스템 정보 표시 | 🔴 결함 | 사용자가 잘못된 기대를 갖고 설치 | 1시간 |
| [A-2](#a-2-envexample이-안내하는-설정이-docker에서-동작하지-않음) | 문서대로 설정해도 동작 안 함 | 🔴 결함 | 설정 13개가 Docker에서 무효 | 15분 |
| [A-3](#a-3-백엔드-에러-메시지가-한국어-전용-56135) | 에러 메시지 한국어 전용 (56/135) | 🟠 품질 | 11개 언어 사용자가 한글 토스트를 봄 | 반나절 |
| [A-4](#a-4-한국어-외-11개-로케일이-170개-문자열-뒤처짐) | 로케일 11개가 ~170개 문자열 누락 | 🟠 품질 | UI 약 30%가 영어로 샘 | 반나절 |
| [B-1](#b-1-대용량-파일을-통째로-메모리에-적재) | 대용량 파일 메모리 적재 | 🔴 위험 | Pi에서 최대 1 GB 스파이크 | 30분 |
| [B-2](#b-2-ci가-릴리스-태그에서만-테스트를-돈다) | CI가 태그에서만 테스트 | 🔴 위험 | 254커밋이 무검증 통과 | 10분 |
| [B-3](#b-3-python-의존성-미고정) | Python 의존성 미고정 | 🟠 위험 | 같은 커밋이 다른 이미지를 만듦 | 20분 |
| [B-4](#b-4-테스트가-12-gb-실파일을-만든다) | 테스트가 1.2 GB 실파일 생성 | 🟠 위험 | tmpfs 고갈 · RAM 1 GiB 스파이크 | **10분** |
| [C-1](#c-1-큰-파일이-곧-변경이-잦은-파일)–[C-7](#c-7-npm-audit-3건--전부-dev-의존성) | 유지보수성 7건 | 🟡 부채 | 계속 비용을 무는 것들 | — |

- [A. 확인된 결함](#a-확인된-결함) — 실제로 잘못 동작하거나 문서와 다른 것
...
> [!WARNING]
> `lynx`가 문제다. 데모의 기본(공식) 뷰에서 Atari Lynx가 **정식 지원 시스템으로 보이지만**,
> 실제 기본 설치(`GNW_EXPERIMENTAL_MODE=false`)에서는 목록에 뜨지 않는다.
> README가 "up to Atari Lynx"를 공식 지원 범위로 안내하고 있어 혼동이 더 커진다.
...
def test_demo_systems_match_backend():
    """demo.js의 SYSTEMS는 systems.py에서 복사된 것이다. 어긋나면 라이브 데모가
    거짓말을 한다 — lynx가 공식 지원인 것처럼 보이는 식으로."""
...
| # | 항목 | 근거 | 비용 |
|---|---|---|---|
| 1 | CI 트리거 확대 (PR/main + `npm run build`) | [B-2](#b-2-ci가-릴리스-태그에서만-테스트를-돈다). 나머지 모든 작업의 안전망 | 10분 |
| 2 | `FileResponse` 3곳 교체 | [B-1](#b-1-대용량-파일을-통째로-메모리에-적재). OOM 제거 + Range 지원 | 30분 |
| 3 | 테스트 sparse 파일 전환 | [B-4](#b-4-테스트가-12-gb-실파일을-만든다). 3줄 수정, tmpfs 5 GB 회수 | 10분 |
| 4 | compose `env_file` 추가 | [A-2](#a-2-envexample이-안내하는-설정이-docker에서-동작하지-않음). 문서대로 동작하게 | 15분 |

...
| # | 항목 | 성격 | 영향 | 상태 |
|---|---|---|---|---|
| [A-1](#a-1-데모-시스템-목록이-백엔드와-3건-어긋남) | 데모 시스템 목록이 백엔드와 3건 어긋남 | 🟡 사소 | 데모 페이지 한정 · 앱 동작 영향 없음 | **✅ v1.12.0** |
...
| [A-5](#a-5-재시작이-커버-자동채움을-버린다) | 재시작이 커버 작업을 버림 | 🔴 결함 | SNES 42%가 영구 공백 | **✅ v1.11.7** |
...
> [!TIP]
> **해결됨 (v1.12.0).** 업스트림 v1.4.0 승격 작업 중 `demo.js`를 `systems.py`에서
> 재생성하면서 3건이 함께 맞춰졌고, `test_demo_parity.py`가 다시 어긋나면 스위트를 깨뜨린다.
...
## A-5. 재시작이 커버 자동채움을 버린다

> [!TIP]
> **해결됨 (v1.11.7).** `_resume_covers`가 부팅마다 200건씩 되찾아 온다. 프로브가
> `_resume_gba_probes`로 이미 갖고 있던 것을 커버도 갖게 됐다.

감사 당시엔 못 봤고, 나중에 사용자가 "SNES 포스터 누락 왜 이렇게 많냐"고 해서 드러났다.

| 시스템 | 커버 없음 |
|---|---:|
| **snes** | **953 / 2,281 (41.8%)** |
| 나머지 전부 (nes·gb·md·gba·pce…) | 0% |

커버리지 문제가 아니었다. 5건을 시험 삼아 `POST /autocover`에 넣으니 **9.4초에 5/5**
성공했고, 누락 목록에는 파이널 판타지 5와 F-제로가 있었다.

**원인** — 자동채움 큐는 메모리에만 산다. 재시작이 그걸 비우고, `main.py`의 시동 코드가
남은 것들을 `cover_status='none'`으로 바꾼다. 스피너는 멈추지만 **작업은 버려진다**:
`none`은 "이 게임은 어디에도 커버가 없다"와 구분되지 않아서 다시 시도하는 코드가 없다.
대량 업로드 + 릴리스 때마다의 재시작이 겹치면 라이브러리가 영구히 반쯤 빈 채로 남는다.

같은 문제를 GBA 프로브는 이미 겪었고 `_resume_gba_probes`로 이어받는 경로를 얻었다.
커버만 버리는 쪽에 남아 있었다.

 ...
 */
    ...
    * exec 531/280,896 (100% idle) — CPU 0% of budget
    */

   /* MPAE  포켓몬스터 - 볼륨 1
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 545/280,896 (100% idle) — CPU 0% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 729/280,896 (100% idle) — CPU 0% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 1,465/280,896 (99% idle) — CPU 1% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 1,648/280,896 (99% idle) — CPU 1% of budget
    */

   /* BPZJ  파즈닌 (Pazuninn - Umininn no Puzzle de Nimu)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 2,958/280,896 (99% idle) — CPU 2% of budget
    */

   /* ABME  슈퍼 버스트 어 무브 (Super Bust-A-Move)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 3,855/280,896 (99% idle) — CPU 2% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 6,011/280,896 (98% idle) — CPU 4% of budget
    */

   /* A2UJ  마더 1+2 (Mother 1+2)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 6,037/280,896 (98% idle) — CPU 4% of budget
    */

   {
      // 도날드덕 어드밴스
      //   exec 6,134/280,896 cy/frame (98% idle) — CPU 4% of budget
      "ADKP",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 6,674/280,896 (98% idle) — CPU 4% of budget
    */

   /* A3ZE  스트리트 잼
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 8,035/280,896 (97% idle) — CPU 5% of budget
    */

   /* BGHJ  학교의 괴담 - 백요상의 봉인 (Gakkou no Kaidan - Hyakuyoubako no Fuuin)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 8,266/280,896 (97% idle) — CPU 5% of budget
    */

   /* BGTE  그랜드 테프트 오토 어드밴스 (Grand Theft Auto)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 8,491/280,896 (97% idle) — CPU 5% of budget
    */
...

   {
      ...
      //   exec 8,793/280,896 cy/frame (97% idle) — CPU 5% of budget
      "BCKE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 8,943/280,896 (97% idle) — CPU 6% of budget
    */
...

   {
      ...
      //   exec 10,269/280,896 cy/frame (96% idle) — CPU 6% of budget
      "ASHJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 10,517/280,896 (96% idle) — CPU 7% of budget
    */

   /* A9CE  CT 스페셜 포스 2 - 백 인 더 트렌치스 (CT Special Forces 2 - Back in the Trenches)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 10,690/280,896 (96% idle) — CPU 7% of budget
    */

   {
      // 환상수호전 카드 스토리즈 (Gensou Suikoden - Card Stories)
      //   exec 10,703/280,896 cy/frame (96% idle) — CPU 7% of budget
      "AGKJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 12,624/280,896 (96% idle) — CPU 8% of budget
    */

   /* ANME  남코 뮤지엄 (Namco Museum)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 13,096/280,896 (95% idle) — CPU 8% of budget
    */

   /* A2GE  GT 어드밴스 3 - 프로 콘셉트 레이싱 (GT Advance 3 - Pro Concept Racing)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 14,289/280,896 (95% idle) — CPU 9% of budget
    */

   /* AKIJ  기기괴계 어드밴스
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 14,464/280,896 (95% idle) — CPU 9% of budget
    */

   /* AKOE  킹 오브 파이터즈 EX 네오 블러드 (King of Fighters EX, The - NeoBlood)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 14,612/280,896 (95% idle) — CPU 9% of budget
    */

   /* DEMO  퍼즐보블
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 15,428/280,896 (95% idle) — CPU 10% of budget
    */

   /* BPWP  파워레인저 - 닌자스톰
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 15,966/280,896 (94% idle) — CPU 10% of budget
    */

   /* AHZJ  피안화 (Higanbana)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 15,992/280,896 (94% idle) — CPU 10% of budget
    */

   {
      // 도쿄뮤뮤 (Hamepane - Tokyo Mew Mew)
      //   exec 16,135/280,896 cy/frame (94% idle) — CPU 10% of budget
      "AM7J",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
...

   {
      ...
      //   exec 17,119/280,896 cy/frame (94% idle) — CPU 11% of budget
      "A3CE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 18,006/280,896 (94% idle) — CPU 11% of budget
    */

   /* AN6E  클로노아 2 드림 챔피언 토너먼트 (Klonoa 2 - Dream Champ Tournament)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 18,062/280,896 (94% idle) — CPU 11% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 19,270/280,896 (93% idle) — CPU 12% of budget
    */

   /* BZIE  니모를 찾아서 - 계속되는 모험
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 19,479/280,896 (93% idle) — CPU 12% of budget
    */

   /* AOGJ  슈퍼로봇대전 OG (Super Robot Taisen - Original Generation) (5)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 19,751/280,896 (93% idle) — CPU 12% of budget
    */

   /* B2CE  팩맨 월드 2 (Pac-Man World 2)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 20,647/280,896 (93% idle) — CPU 13% of budget
    */

   /* AKCE  코나미 아케이드게임 컬렉션
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 21,153/280,896 (92% idle) — CPU 13% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 22,019/280,896 (92% idle) — CPU 14% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 22,155/280,896 (92% idle) — CPU 14% of budget
    */

   /* BRDE  파워레인저 - SPD (Power Rangers S.P.D.)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 22,976/280,896 (92% idle) — CPU 14% of budget
    */
...

   {
      ...
      //   exec 25,802/280,896 cy/frame (91% idle) — CPU 16% of budget
      "APDE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 26,499/280,896 (91% idle) — CPU 17% of budget
    */

   {
      // 마리오 테니스 파워 투어 (Mario Tennis - Power Tour)
      //   exec 26,988/280,896 cy/frame (90% idle) — CPU 17% of budget
      "BTME",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
...

   {
      ...
      //   exec 27,068/280,896 cy/frame (90% idle) — CPU 17% of budget
      "ABTE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 27,389/280,896 (90% idle) — CPU 17% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 28,165/280,896 (90% idle) — CPU 18% of budget
    */

   /* AONE  버블 보블 - 올드 앤 뉴 (Bubble Bobble - Old & New)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 28,491/280,896 (90% idle) — CPU 18% of budget
    */

   /* BNFE  니드 포 스피드 언더그라운드 2 (Need for Speed - Underground 2)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 29,331/280,896 (90% idle) — CPU 18% of budget
    */

   /* AB9E  듀얼 블레이즈
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 29,518/280,896 (89% idle) — CPU 18% of budget
    */

   /* BPBJ  삐리리 불어봐 재규어!
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 29,728/280,896 (89% idle) — CPU 19% of budget
    */

   /* AKOJ  킹 오브 파이터즈 EX 네오 블러드 + 33 NES
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 29,808/280,896 (89% idle) — CPU 19% of budget
    */

   /* AKUJ  흑수염의 쿠룻또 진토리 (Kurohige no Kurutto Jintori)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 30,241/280,896 (89% idle) — CPU 19% of budget
    */

   /* AD2J  미스터 드릴러 2 (Mr. Driller 2)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 30,950/280,896 (89% idle) — CPU 19% of budget
    */

   {
      // 고인돌
      //   exec 30,968/280,896 cy/frame (89% idle) — CPU 19% of budget
      "APHE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 31,018/280,896 (89% idle) — CPU 19% of budget
    */

   /* BOJJ  오자루마루 - 달빛 마을 투어 드 오자루 (Ojarumaru - Gekkouchou Sanpo de Ojaru)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 31,426/280,896 (89% idle) — CPU 20% of budget
    */

   /* AGGE  그레믈린 스트라이프 대 기즈모 (Gremlins - Stripe vs Gizmo)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 31,583/280,896 (89% idle) — CPU 20% of budget
    */

   /* AP9P  포켓 뮤직 (Pocket Music)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 31,672/280,896 (89% idle) — CPU 20% of budget
    */

   /* BN4J  강의 낚시3,4 (Kawa no Nushi Tsuri 3 & 4)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 31,994/280,896 (89% idle) — CPU 20% of budget
    */

   /* A8ZJ  진 여신전생 - 퍼즐데콜 (Shin Megami Tensei - Devil Children - Puzzle de Call!)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 32,699/280,896 (88% idle) — CPU 20% of budget
    */

   /* AMBJ  모바일 프로야구 (Mobile Pro Yakyuu - Kantoku no Saihai)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 32,713/280,896 (88% idle) — CPU 20% of budget
    */

   /* BVGJ  비트 제너레이션즈 사운드 보이저 (bit Generations - Soundvoyager)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 33,020/280,896 (88% idle) — CPU 21% of budget
    */

   /* BAMJ  내일의 죠 - 새빨갛게 불타올라라
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 33,026/280,896 (88% idle) — CPU 21% of budget
    */

   /* AKLJ  바람의 크로노아
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 34,537/280,896 (88% idle) — CPU 22% of budget
    */

   /* AD5J  미스터 드릴러 에이스
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 34,701/280,896 (88% idle) — CPU 22% of budget
    */

   /* AD4E  던전 앤 드래곤 - 비홀더의 눈 (Dungeons & Dragons - Eye of the Beholder)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 35,278/280,896 (87% idle) — CPU 22% of budget
    */

   /* A9DE  둠II (Doom II)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 35,456/280,896 (87% idle) — CPU 22% of budget
    */

   {
      // 타이니 툰 어드벤처 - 와키 스태커즈 (Tiny Toon Adventures - Wacky Stackers)
      //   exec 35,606/280,896 cy/frame (87% idle) — CPU 22% of budget
      "AWSE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 35,702/280,896 (87% idle) — CPU 22% of budget
    */

   {
      // 스타워즈 - 제다이 파워 배틀 (Star Wars - Jedi Power Battles)
      //   exec 36,014/280,896 cy/frame (87% idle) — CPU 23% of budget
      "ASWE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 36,894/280,896 (87% idle) — CPU 23% of budget
    */

   {
      // 쵸로Q어드밴스2 (Choro Q Advance 2)
      //   exec 37,276/280,896 cy/frame (87% idle) — CPU 23% of budget
      "AQ2J",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 37,449/280,896 (87% idle) — CPU 23% of budget
    */

   /* BIJE  소닉 더 헤지혹
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 38,536/280,896 (86% idle) — CPU 24% of budget
    */

   /* BDGE  디지몬 레이싱 (Digimon Racing)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 39,279/280,896 (86% idle) — CPU 25% of budget
    */

   /* AISJ  월희
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 40,438/280,896 (86% idle) — CPU 25% of budget
    */

   /* BSGJ  모두의 소프트 시리즈 - 모두의 쇼기
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 41,232/280,896 (85% idle) — CPU 26% of budget
    */

   {
      // TMNT (TMNT)
      //   exec 41,238/280,896 cy/frame (85% idle) — CPU 26% of budget
      "BEXE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 41,512/280,896 (85% idle) — CPU 26% of budget
    */

   /* HGRS  히구라시의 울음소리
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 41,631/280,896 (85% idle) — CPU 26% of budget
    */

   {
      // 라라 크로프트 툼레이더 - The Prophecy
      //   exec 42,011/280,896 cy/frame (85% idle) — CPU 26% of budget
      "AUTJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 42,503/280,896 (85% idle) — CPU 27% of budget
    */

   /* ACAE  GT 챔피언십 레이싱 (GT Advance - Championship Racing)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 43,058/280,896 (85% idle) — CPU 27% of budget
    */

   /* A3KE  인터내셔널 가라테 플러스 (IK+)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 43,965/280,896 (84% idle) — CPU 27% of budget
    */

   /* BH6J  건담 하로 뿌요뿌요 (Kidou Gekidan Haro Ichiza - Haro no Puyo Puyo)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 44,269/280,896 (84% idle) — CPU 28% of budget
    */

   /* AARE  수왕기 (Altered Beast - Guardian of the Realms)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 44,684/280,896 (84% idle) — CPU 28% of budget
    */

   /* ATIJ  테니스의 왕자 - 천재소년 아카데미
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 45,410/280,896 (84% idle) — CPU 28% of budget
    */

   /* AWIJ  하이퍼스포츠 2002 동계 (Hyper Sports 2002 Winter)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 45,611/280,896 (84% idle) — CPU 29% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 48,433/280,896 (83% idle) — CPU 30% of budget
    */

   /* BKTJ  강철제국 (Koutetsu Teikoku)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 48,738/280,896 (83% idle) — CPU 30% of budget
    */

   /* BQXE  슈퍼맨 리턴즈 - 고독의 요새 (Superman Returns - Fortress of Solitude)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 48,794/280,896 (83% idle) — CPU 30% of budget
    */

   /* A3MJ  미키와 미니의 매지컬퀘스트
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 49,154/280,896 (83% idle) — CPU 31% of budget
    */

   /* BITJ  음양대전기 제로식 (Onmyou Taisenki - Zeroshiki)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 49,290/280,896 (82% idle) — CPU 31% of budget
    */

   /* BHVE  스쿼지 하이브
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 49,346/280,896 (82% idle) — CPU 31% of budget
    */

   /* BUFE  2-in-1 드래곤볼 Z 게임팩 - 부우의 분노 & GT 트랜스포메이션
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 49,705/280,896 (82% idle) — CPU 31% of budget
    */

   /* AQAE  가제트 레이서스 (Gadget Racers)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 50,826/280,896 (82% idle) — CPU 32% of budget
    */

   /* A9CP  CT 스페셜 포스 2 - 백 투 헬 (CT Special Forces - Back to Hell)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 50,836/280,896 (82% idle) — CPU 32% of budget
    */

   /* AGSJ  황금의 태양
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 50,863/280,896 (82% idle) — CPU 32% of budget
    */

   /* BKTP  스틸 엠파이어 (Steel Empire)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 51,173/280,896 (82% idle) — CPU 32% of budget
    */

   /* BO8K  원피스 - 고잉 베이스볼 (One Piece - Going Baseball - Haejeok Yaku)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 51,174/280,896 (82% idle) — CPU 32% of budget
    */

   /* BT2E  틴에이지 뮤턴트 닌자 터틀즈 2 - 배틀 넥서스 (Teenage Mutant Ninja Turtles 2 - Battle Nexus)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 51,382/280,896 (82% idle) — CPU 32% of budget
    */

   /* AIDE  스페이스 인베이더 (Space Invaders)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 51,652/280,896 (82% idle) — CPU 32% of budget
    */

   /* AQAP  페니 레이서스 (Penny Racers)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 51,760/280,896 (82% idle) — CPU 32% of budget
    */

   /* AUYJ  유령저택의 24시간 (Yuureiyashiki no Nijuuyojikan)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 52,425/280,896 (81% idle) — CPU 33% of budget
    */

   /* BBKJ  부라부라동키 (Bura Bura Donkey)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 52,571/280,896 (81% idle) — CPU 33% of budget
    */

   /* AHKJ  히카루의 바둑
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 53,043/280,896 (81% idle) — CPU 33% of budget
    */

   /* ASYE  스파이로 - 시즌 오브 아이스
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 53,336/280,896 (81% idle) — CPU 33% of budget
    */

   /* U3IJ  우리들의 태양
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 53,548/280,896 (81% idle) — CPU 33% of budget
    */

   /* AEGE  익스트림 고스트 버스터즈 - 코드 에크토-1 (Extreme Ghostbusters - Code Ecto-1)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 54,193/280,896 (81% idle) — CPU 34% of budget
    */

   {
      // 쿠루쿠루쿠루링
      //   exec 54,364/280,896 cy/frame (81% idle) — CPU 34% of budget
      "AKRJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
...

   {
      ...
      //   exec 54,570/280,896 cy/frame (81% idle) — CPU 34% of budget
      "AKRP",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
      0x8000422,                   /* idle_loop_target_pc  */
      0,                           /* translation_gate_target_1 */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 54,835/280,896 (80% idle) — CPU 34% of budget
    */

   /* BR8E  고스트라이더 (Ghost Rider)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 55,497/280,896 (80% idle) — CPU 35% of budget
    */

   /* A8VJ  보보보보 보보보 - 오우기 87.5 바쿠레츠 하나게 신켄 (Boboboubo Boubobo - Ougi 87.5 Bakuretsu Hanage Shinken)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 55,503/280,896 (80% idle) — CPU 35% of budget
    */

   /* AGWP  GT 어드밴스 2 - 랠리 레이싱 (GT Advance 2 - Rally Racing)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 55,763/280,896 (80% idle) — CPU 35% of budget
    */

   /* AGWE  GT 어드밴스 2 - 랠리 레이싱 + 98 NES
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 55,867/280,896 (80% idle) — CPU 35% of budget
    */

   {
      // 다운타운 열혈물어EX (한글패치)
      //   exec 55,917/280,896 cy/frame (80% idle) — CPU 35% of budget
      "BDTE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 55,939/280,896 (80% idle) — CPU 35% of budget
    */

   /* BG8J  간바레! 도지 파이터즈 (Ganbare! Dodge Fighters)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 55,959/280,896 (80% idle) — CPU 35% of budget
    */

   /* AK4J  근육랭킹4 (Kinniku Banzuke - Kongou-kun no Daibouken!)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 56,204/280,896 (80% idle) — CPU 35% of budget
    */

   /* AQAJ  쵸로Q 어드밴스 (Choro Q Advance)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 57,269/280,896 (80% idle) — CPU 36% of budget
    */

   /* ALFE  드래곤볼 Z - 오공의 유산 II (Dragon Ball Z - The Legacy of Goku II)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 57,393/280,896 (80% idle) — CPU 36% of budget
    */

   /* BLFE  2-in-1 드래곤볼 Z 게임팩 - 오공의 유산 1 & 2
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 57,483/280,896 (80% idle) — CPU 36% of budget
    */

   /* ABDJ  볼더 대쉬 EX (Boulder Dash EX)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 57,649/280,896 (79% idle) — CPU 36% of budget
    */

   /* AJFE  정글북 (Jungle Book, The)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 57,694/280,896 (79% idle) — CPU 36% of budget
    */

   /* BOSJ  보보보보 보보보 - 바쿠텐 하지케 타이센 (Boboboubo Boubobo - Bakutou Hajike Taisen)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 57,762/280,896 (79% idle) — CPU 36% of budget
    */

   /* BDDJ  더블 드래곤 어드밴스 (Double Dragon Advance)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 57,778/280,896 (79% idle) — CPU 36% of budget
    */

   /* AJFP  디즈니 정글북 2 (Jungle Book 2, The)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 58,389/280,896 (79% idle) — CPU 36% of budget
    */

   {
      // 데어데블
      //   exec 58,617/280,896 cy/frame (79% idle) — CPU 37% of budget
      "AVLE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 58,931/280,896 (79% idle) — CPU 37% of budget
    */

   /* AHUE  샤이닝소울
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 59,298/280,896 (79% idle) — CPU 37% of budget
    */

   /* BBGE  배트맨 비긴즈 (Batman Begins)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 59,447/280,896 (79% idle) — CPU 37% of budget
    */

   /* ATCE  탑 기어 GT 챔피언십 (Top Gear GT Championship)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 59,678/280,896 (79% idle) — CPU 37% of budget
    */
...

   {
      ...
      //   exec 59,709/280,896 cy/frame (79% idle) — CPU 37% of budget
      "AFZJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
...

   {
      ...
      //   exec 59,738/280,896 cy/frame (79% idle) — CPU 37% of budget
      "AFZE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
      0x8000c2e,                   /* idle_loop_target_pc  */
      0,                           /* translation_gate_target_1 */
...

   {
      ...
      //   exec 59,925/280,896 cy/frame (79% idle) — CPU 37% of budget
      "AMQE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 60,090/280,896 (79% idle) — CPU 38% of budget
    */

   /* AM4P  모토GP
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 60,347/280,896 (79% idle) — CPU 38% of budget
    */

   /* AMXD  몬스터 주식회사 (Monster AG, Die)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 60,393/280,896 (78% idle) — CPU 38% of budget
    */

   /* BSPE  스파이더맨2
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 60,603/280,896 (78% idle) — CPU 38% of budget
    */

   {
      // 실황 월드사커 포켓2 (Jikkyou World Soccer Pocket 2)
      //   exec 60,688/280,896 cy/frame (78% idle) — CPU 38% of budget
      "AJKJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 60,749/280,896 (78% idle) — CPU 38% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 61,550/280,896 (78% idle) — CPU 38% of budget
    */

   {
      // 초마계촌R
      //   exec 61,618/280,896 cy/frame (78% idle) — CPU 39% of budget
      "ACJJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
   ...
   {
      // 페르시아의 왕자 - 시간의 모래
      //   exec 61,756/280,896 cy/frame (78% idle) — CPU 39% of budget
      "BPYP",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 61,959/280,896 (78% idle) — CPU 39% of budget
    */

   /* AMRE  Motocross Maniacs Advance (U) [!]
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 62,170/280,896 (78% idle) — CPU 39% of budget
    */

   /* AZBJ  배스츠리 시요우제 (Bass Tsuri Shiyouze! - Tournament wa Senryaku da!)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 62,200/280,896 (78% idle) — CPU 39% of budget
    */

   /* AMTJ  메트로이드 - 퓨전 (Metroid Fusion)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 62,335/280,896 (78% idle) — CPU 39% of budget
    */

   /* AWAE  와리오 랜드 4 (Wario Land 4)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 62,403/280,896 (78% idle) — CPU 39% of budget
    */

   {
      // 베이스볼 어드밴스 (Baseball Advance)
      //   exec 62,420/280,896 cy/frame (78% idle) — CPU 39% of budget
      "ABPE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 62,585/280,896 (78% idle) — CPU 39% of budget
    */

   /* AWAJ  와리오랜드 어드밴스
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 62,602/280,896 (78% idle) — CPU 39% of budget
    */

   /* A4NJ  목장이야기 - 미네랄타운의 친구들
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 62,670/280,896 (78% idle) — CPU 39% of budget
    */

   /* BMOJ  미나노 오지사마 (Minna no Ouji-sama)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 63,275/280,896 (77% idle) — CPU 40% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 63,989/280,896 (77% idle) — CPU 40% of budget
    */

   {
      // 에프제로 - 팔콘 전설 (F-Zero - Falcon Densetsu)
      //   exec 64,359/280,896 cy/frame (77% idle) — CPU 40% of budget
      "BFZJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 64,410/280,896 (77% idle) — CPU 40% of budget
    */

   {
      // 에프제로 GP 레전드 (F-Zero - GP Legend)
      //   exec 64,480/280,896 cy/frame (77% idle) — CPU 40% of budget
      "BFZE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
      0x8000c32,                   /* idle_loop_target_pc  */
      0,                           /* translation_gate_target_1 */
   ...
   {
      // 스페이스 인베이더EX
      //   exec 64,578/280,896 cy/frame (77% idle) — CPU 40% of budget
      "AIDJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 64,902/280,896 (77% idle) — CPU 41% of budget
    */

   /* BABJ  알렉보던 어드밴쳐 - 타워&샤프트 어드밴스
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 64,904/280,896 (77% idle) — CPU 41% of budget
    */

   /* AMRJ  모토크로스 매니악스 어드밴스 (Motocross Maniacs Advance)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 65,241/280,896 (77% idle) — CPU 41% of budget
    */
...

   {
      ...
      //   exec 65,659/280,896 cy/frame (77% idle) — CPU 41% of budget
      "AR6E",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 66,442/280,896 (76% idle) — CPU 42% of budget
    */

   {
      // 짱구는 못말려 - 쇼크 가언의 인형들에 대항하여 (Shin chan contra los Munecos de Shock Gahn)
      //   exec 67,175/280,896 cy/frame (76% idle) — CPU 42% of budget
      "BC2S",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 67,239/280,896 (76% idle) — CPU 42% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 67,733/280,896 (76% idle) — CPU 42% of budget
    */

   {
      // 댄싱스워드 - 섬광
      //   exec 67,766/280,896 cy/frame (76% idle) — CPU 42% of budget
      "A9SJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 67,824/280,896 (76% idle) — CPU 42% of budget
    */

   /* BPPP  Pokemon Pinball - Ruby & Sapphire (E) (M5)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 67,968/280,896 (76% idle) — CPU 42% of budget
    */

   /* AF3J  제로 원 (Zero One)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 67,973/280,896 (76% idle) — CPU 42% of budget
    */

   /* B4KJ  네모난 머리를 둥글게 어드밴스 - 한자 계산 (Shikakui Atama o Maruku Suru. Advance - Kanji, Keisan)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 67,988/280,896 (76% idle) — CPU 42% of budget
    */

   /* ALFJ  드래곤볼 Z - 오공의 유산 II 인터내셔널 (Dragon Ball Z - The Legacy of Goku II International)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 68,286/280,896 (76% idle) — CPU 43% of budget
    */

   {
      // 핀볼 챌린지 디럭스 (Pinball Challenge Deluxe)
      //   exec 68,413/280,896 cy/frame (76% idle) — CPU 43% of budget
      "APLP",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 68,540/280,896 (76% idle) — CPU 43% of budget
    */

   /* AKZJ  카마이타치의 밤 어드밴스 (Kamaitachi no Yoru Advance)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 68,901/280,896 (75% idle) — CPU 43% of budget
    */

   {
      // 캡틴 츠바사 - 영광의 기적 (Captain Tsubasa - Eikou no Kiseki)
      //   exec 69,325/280,896 cy/frame (75% idle) — CPU 43% of budget
      "AKYJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
...

   {
      ...
      //   exec 69,569/280,896 cy/frame (75% idle) — CPU 43% of budget
      "B2DJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 69,584/280,896 (75% idle) — CPU 43% of budget
    */

   {
      // 동키콩 컨트리 2 (Donkey Kong Country 2)
      //   exec 69,587/280,896 cy/frame (75% idle) — CPU 43% of budget
      "B2DE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
      0x80003d4,                   /* idle_loop_target_pc  */
      0,                           /* translation_gate_target_1 */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 70,513/280,896 (75% idle) — CPU 44% of budget
    */

   {
      // 별의 커비 - 꿈의 샘 디럭스 (Kirby - Nightmare in Dream Land)
      //   exec 70,574/280,896 cy/frame (75% idle) — CPU 44% of budget
      "A7KE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 71,162/280,896 (75% idle) — CPU 44% of budget
    */

   /* AAWE  혼두라 하드 스피릿츠
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 71,391/280,896 (75% idle) — CPU 45% of budget
    */

   {
      // 마리오 테니스 어드밴스 (Mario Tennis Advance)
      //   exec 71,646/280,896 cy/frame (74% idle) — CPU 45% of budget
      "BTMJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 72,217/280,896 (74% idle) — CPU 45% of budget
    */

   {
      // 봄버맨 제터즈
      //   exec 72,267/280,896 cy/frame (74% idle) — CPU 45% of budget
      "AJZJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 72,673/280,896 (74% idle) — CPU 45% of budget
    */

   /* BT8E  틴에이지 뮤턴트 닌자 터틀즈 더블 팩 (Teenage Mutant Ninja Turtles Double Pack)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 72,753/280,896 (74% idle) — CPU 45% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 72,872/280,896 (74% idle) — CPU 46% of budget
    */
...

   {
      ...
      //   exec 73,351/280,896 cy/frame (74% idle) — CPU 46% of budget
      "AFZC",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 73,493/280,896 (74% idle) — CPU 46% of budget
    */

   /* BRWP  레이싱 피버 (Racing Fever)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 73,736/280,896 (74% idle) — CPU 46% of budget
    */

   /* AAWP  콘트라 어드밴스 - 더 에일리언 워즈 EX (Contra Advance - The Alien Wars EX)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 73,846/280,896 (74% idle) — CPU 46% of budget
    */

   /* BOBJ  보보보보 보보보 2 (Boboboubo Boubobo - Maji de!! Shinken Battle)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 74,123/280,896 (74% idle) — CPU 46% of budget
    */

   {
      // 마리오 vs. 동키콩 (Mario vs. Donkey Kong)
      //   exec 74,248/280,896 cy/frame (74% idle) — CPU 46% of budget
      "BM5J",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 74,519/280,896 (73% idle) — CPU 47% of budget
    */

   /* BIXJ  칼쵸비트 (Calciobit)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 74,535/280,896 (73% idle) — CPU 47% of budget
    */

   /* BIIJ  통근일필 (Tsuukin Hitofude)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 74,621/280,896 (73% idle) — CPU 47% of budget
    */

   {
      // 포켓몬스터 리프그린 (한글패치, 미국판 헤더)
      //   exec 74,872/280,896 cy/frame (73% idle) — CPU 47% of budget
      "BPGE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 75,413/280,896 (73% idle) — CPU 47% of budget
    */

   /* BGKJ  게게게의 귀태랑 - 위기일발! 요괴열도 (Gegege no Kitarou - Kikiippatsu! Youkai Rettou)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 75,498/280,896 (73% idle) — CPU 47% of budget
    */
...

   {
      ...
      //   exec 76,369/280,896 cy/frame (73% idle) — CPU 48% of budget
      "AAMJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
...

   {
      ...
      //   exec 76,509/280,896 cy/frame (73% idle) — CPU 48% of budget
      "BPRE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
      0x80008c6,                   /* idle_loop_target_pc  */
      0,                           /* translation_gate_target_1 */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 76,516/280,896 (73% idle) — CPU 48% of budget
    */

   /* A7AJ  나루토
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 77,295/280,896 (72% idle) — CPU 48% of budget
    */

   /* BG2J  고에몽1,2 (Kessaku Sen! - Ganbare Goemon 1, 2 - Yuki Hime to Magginesu)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 77,415/280,896 (72% idle) — CPU 48% of budget
    */

   /* BMVP  슈퍼 마리오 볼 (Super Mario Ball)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 77,689/280,896 (72% idle) — CPU 49% of budget
    */

   /* BUOJ  모두의 소프트 시리즈 - 난프레 어드밴스 (Minna no Soft Series - Numpla Advance)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 77,829/280,896 (72% idle) — CPU 49% of budget
    */

   /* ARVJ  레이브 - 빛과 그림자의 대결전 (Groove Adventure Rave - Hikari to Yami no Daikessen)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 77,950/280,896 (72% idle) — CPU 49% of budget
    */

   {
      // 리듬세상 (한글패치)
      //   exec 78,061/280,896 cy/frame (72% idle) — CPU 49% of budget
      "BRIJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 78,274/280,896 (72% idle) — CPU 49% of budget
    */

   /* AWVE  엑스맨 2 - 울버린의 복수 (X2 - Wolverine's Revenge)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 78,277/280,896 (72% idle) — CPU 49% of budget
    */

   {
      // 포켓몬스터 에메랄드 (정식 한국판)
      //   exec 78,294/280,896 cy/frame (72% idle) — CPU 49% of budget
      "BPEK",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 78,324/280,896 (72% idle) — CPU 49% of budget
    */

   {
      // 스타워즈 - 디 뉴 드로이드 아미 (Star Wars - The New Droid Army)
      //   exec 78,358/280,896 cy/frame (72% idle) — CPU 49% of budget
      "A2WE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 78,684/280,896 (72% idle) — CPU 49% of budget
    */

   /* AEXJ  더 킹 오브 파이터즈 EX2 - 하울링 블러드 (The King of Fighters EX2 - Howling Blood)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 78,738/280,896 (72% idle) — CPU 49% of budget
    */

   /* A3VJ  소닉 핀볼파티
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 78,939/280,896 (72% idle) — CPU 49% of budget
    */

   /* ALGE  드래곤볼 Z - 손오공의 유산 (Dragon Ball Z - The Legacy of Goku)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 79,223/280,896 (72% idle) — CPU 50% of budget
    */

   /* A9LJ  테니스의 왕자 2003 - 쿨 블루 (Tennis no Ouji-sama 2003 - Cool Blue)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 79,614/280,896 (72% idle) — CPU 50% of budget
    */

   {
      // 드래곤볼 Z - 무공투극 (Dragon Ball Z - Taiketsu)
      //   exec 79,640/280,896 cy/frame (72% idle) — CPU 50% of budget
      "BDBE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 79,928/280,896 (72% idle) — CPU 50% of budget
    */

   /* AN7J  패미스타 어드밴스 (Famista Advance)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 80,200/280,896 (71% idle) — CPU 50% of budget
    */

   /* AG4J  고지라 괴수대단투 어드밴스 (Gojira - Kaijuu Dairantou Advance)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 80,771/280,896 (71% idle) — CPU 50% of budget
    */

   /* A2OJ  K-1 포켓 그랑프리2 (K-1 Pocket Grand Prix 2)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 80,809/280,896 (71% idle) — CPU 51% of budget
    */

   /* AAWJ  콘트라 하드 스피리츠 (Contra Hard Spirits)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 80,902/280,896 (71% idle) — CPU 51% of budget
    */

   {
      // 조이드 사가 2 (Zoids Saga II)
      //   exec 81,163/280,896 cy/frame (71% idle) — CPU 51% of budget
      "AZ2J",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 81,421/280,896 (71% idle) — CPU 51% of budget
    */

   /* ALNE  루나 레전드 (Lunar Legend)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 81,621/280,896 (71% idle) — CPU 51% of budget
    */

   /* FSME  슈퍼 마리오 브라더스 클래식 (Classic NES Series - Super Mario Bros.)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 82,365/280,896 (71% idle) — CPU 51% of budget
    */

   /* BBSJ  보우캬쿠노 센리츠 (Boukyaku no Senritsu)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 82,534/280,896 (71% idle) — CPU 52% of budget
    */

   /* ADEJ  도쿄 디즈니 씨 모험 (Adventure of Tokyo Disney Sea)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 82,737/280,896 (71% idle) — CPU 52% of budget
    */

   /* AE7J  파이어 엠블렘 - 열화의 검 (Fire Emblem - Rekka no Ken)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 82,919/280,896 (70% idle) — CPU 52% of budget
    */

   {
      // 그레이티스트 나인 (Greatest Nine)
      //   exec 83,073/280,896 cy/frame (70% idle) — CPU 52% of budget
      "AG9J",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 84,612/280,896 (70% idle) — CPU 53% of budget
    */

   /* AKVJ  K-1 포켓 그랑프리 (K-1 Pocket Grand Prix)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 85,005/280,896 (70% idle) — CPU 53% of budget
    */

   {
      // 쥬라기공원III - 잃어버린 유전자
      //   exec 85,016/280,896 cy/frame (70% idle) — CPU 53% of budget
      "ADNE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 85,133/280,896 (70% idle) — CPU 53% of budget
    */

   /* A8TJ  RPG 츠쿠르 어드밴스 (RPG Tsukuru Advance)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 85,562/280,896 (70% idle) — CPU 53% of budget
    */

   {
      // 마리오 vs. 동키콩 (Mario vs. Donkey Kong)
      //   exec 85,587/280,896 cy/frame (70% idle) — CPU 53% of budget
      "BM5E",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 86,382/280,896 (69% idle) — CPU 54% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 87,460/280,896 (69% idle) — CPU 55% of budget
    */

   /* AU2J  샤이닝 소울 2 (Shining Soul II)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 87,558/280,896 (69% idle) — CPU 55% of budget
    */

   /* ASXJ  삼국지0.75
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 87,625/280,896 (69% idle) — CPU 55% of budget
    */

   /* A3UJ  마더 3 (Mother 3)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 87,667/280,896 (69% idle) — CPU 55% of budget
    */

   /* SBFP  부타노파이터 (Butano Fighter)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 87,737/280,896 (69% idle) — CPU 55% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 88,962/280,896 (68% idle) — CPU 56% of budget
    */

   /* ASOJ  소닉 어드밴스 1 (Sonic Advance)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 89,177/280,896 (68% idle) — CPU 56% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 89,671/280,896 (68% idle) — CPU 56% of budget
    */
...

   {
      ...
      //   exec 90,148/280,896 cy/frame (68% idle) — CPU 56% of budget
      "AGFJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 90,283/280,896 (68% idle) — CPU 56% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 91,625/280,896 (67% idle) — CPU 57% of budget
    */

   /* BT3J  탐정 진구지 사부로 - 하얀 그림자의 소녀 (Tantei Jinguuji Saburou - Shiroi Kage no Shoujo)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 92,117/280,896 (67% idle) — CPU 58% of budget
    */

   /* APXJ  파랑스 - The Enforce Fighter A-144
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 92,813/280,896 (67% idle) — CPU 58% of budget
    */

   /* APXE  팔랑크스 - 인포스 파이터 A-144 (Phalanx)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 93,112/280,896 (67% idle) — CPU 58% of budget
    */

   {
      // 얼티밋 브레인 게임즈 (Ultimate Brain Games)
      //   exec 93,471/280,896 cy/frame (67% idle) — CPU 58% of budget
      "ABUE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
...

   {
      ...
      //   exec 94,167/280,896 cy/frame (66% idle) — CPU 59% of budget
      "AGAE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 94,542/280,896 (66% idle) — CPU 59% of budget
    */

   /* BANE  반 헬싱 (Van Helsing)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 94,613/280,896 (66% idle) — CPU 59% of budget
    */

   /* A2NP  소닉 어드밴스 2 (Sonic Advance 2)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 94,828/280,896 (66% idle) — CPU 59% of budget
    */

   {
      // 록맨 제로 1 (Megaman Zero 1)
      //   exec 94,981/280,896 cy/frame (66% idle) — CPU 59% of budget
      "ARZJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
...

   {
      ...
      //   exec 96,147/280,896 cy/frame (66% idle) — CPU 60% of budget
      "BSME",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 96,227/280,896 (66% idle) — CPU 60% of budget
    */

   /* AJCF  재키 찬의 모험 (Aventures de Jackie Chan, Les - La Legende de la Main Noire)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 96,278/280,896 (66% idle) — CPU 60% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 98,159/280,896 (65% idle) — CPU 61% of budget
    */

   /* ANYJ  가친코 프로야구 (Gachinko Pro Yakyuu)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 98,414/280,896 (65% idle) — CPU 62% of budget
    */

   /* BAGJ  어드밴스 가디언 히어로즈 (Advance Guardian Heroes)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 98,498/280,896 (65% idle) — CPU 62% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 99,277/280,896 (65% idle) — CPU 62% of budget
    */

   /* AJ9J  슈퍼로봇대전 OG (Super Robot Taisen - Original Generation) (2)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 99,448/280,896 (65% idle) — CPU 62% of budget
    */

   /* BR9J  리락쿠마한 매일 (Relaxuma na Mainichi)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 99,739/280,896 (64% idle) — CPU 62% of budget
    */

   /* AIOE  아이언맨 (Invincible Iron Man, The)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 99,755/280,896 (64% idle) — CPU 62% of budget
    */

   /* AGNJ  고에몽 - 뉴에이지출동! (Goemon - New Age Shutsudou!)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 99,758/280,896 (64% idle) — CPU 62% of budget
    */

   /* BI3E  스파이더맨 3 (Spider-Man 3)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 99,833/280,896 (64% idle) — CPU 62% of budget
    */

   /* AG4E  고질라 도미네이션 (Godzilla - Domination!)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 99,845/280,896 (64% idle) — CPU 62% of budget
    */

   /* BLWJ  레고 스타 워즈 더 비디오 게임 (LEGO Star Wars - The Video Game)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 100,444/280,896 (64% idle) — CPU 63% of budget
    */

   /* ATTP  타이니툰 어드밴쳐 - Buster's Bad Dream
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 100,609/280,896 (64% idle) — CPU 63% of budget
    */

   /* ALEE  브루스 리 - 전설의 귀환 (Bruce Lee - Return of the Legend)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 100,995/280,896 (64% idle) — CPU 63% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 101,903/280,896 (64% idle) — CPU 64% of budget
    */

   /* 2G0P  굿보이 갤럭시 데모
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 101,959/280,896 (64% idle) — CPU 64% of budget
    */

   /* A9KJ  슬라임 모리모리 드래곤 퀘스트 - 충격의 꼬리단 (Slime Morimori Dragon Quest - Shougeki no Shippo Dan)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 102,077/280,896 (64% idle) — CPU 64% of budget
    */

   /* BONE  원피스 소년점프 (One Piece)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 103,046/280,896 (63% idle) — CPU 64% of budget
    */

   {
      // 디즈니 스포츠 - 스노보딩 (Disney Sports - Snowboarding)
      //   exec 103,765/280,896 cy/frame (63% idle) — CPU 65% of budget
      "A5DE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 103,883/280,896 (63% idle) — CPU 65% of budget
    */

   /* AC8E  크래쉬밴디쿳 어드밴스2
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 104,532/280,896 (63% idle) — CPU 65% of budget
    */

   /* BLEJ  블리치 어드밴스
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 104,773/280,896 (63% idle) — CPU 65% of budget
    */

   /* AAPJ  메탈건 슬링거
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 104,777/280,896 (63% idle) — CPU 65% of budget
    */

   /* ARNJ  머나먼 시공속에서 (Neoromance Game - Harukanaru Toki no Naka de)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 106,036/280,896 (62% idle) — CPU 66% of budget
    */

   {
      // 록맨 에그제
      //   exec 106,473/280,896 cy/frame (62% idle) — CPU 67% of budget
      "AREJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
...

   {
      ...
      //   exec 106,583/280,896 cy/frame (62% idle) — CPU 67% of budget
      "BY3J",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 106,854/280,896 (62% idle) — CPU 67% of budget
    */

   {
      // 주큐브 (ZooCube)
      //   exec 106,976/280,896 cy/frame (62% idle) — CPU 67% of budget
      "ANCJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 106,997/280,896 (62% idle) — CPU 67% of budget
    */

   /* BKKJ  모두의 사육 시리즈3 - 나의 장수풍뎅이 사슴벌레
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 107,291/280,896 (62% idle) — CPU 67% of budget
    */

   /* AFEK  파이어 엠블렘 - 봉인의 검 (Fire Emblem Sealed Sword)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 107,691/280,896 (62% idle) — CPU 67% of budget
    */

   /* BDAJ  꽃놀이 퍼즐 어드밴스 (Don-chan Puzzle - Hanabi de Doon! Advance)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 108,163/280,896 (61% idle) — CPU 68% of budget
    */

   /* AN9J  테일즈 오브 월드 - 나리키리던젼2
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 108,240/280,896 (61% idle) — CPU 68% of budget
    */

   /* FM2J  패미콤 미니 - Vol. 21 - 슈퍼 마리오 브라더스 2 (Famicom Mini 21 - Super Mario Bros. 2)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 108,655/280,896 (61% idle) — CPU 68% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 110,098/280,896 (61% idle) — CPU 69% of budget
    */

   {
      // 크레이지 레이서즈 (Konami Krazy Racers)
      //   exec 110,630/280,896 cy/frame (61% idle) — CPU 69% of budget
      "AKWE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
      0x8000422,                   /* idle_loop_target_pc  */
      0,                           /* translation_gate_target_1 */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 111,600/280,896 (60% idle) — CPU 70% of budget
    */

   /* BTAJ  아스트로보이 철완아톰
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 112,034/280,896 (60% idle) — CPU 70% of budget
    */

   {
      // 코나미 와이와이 레이싱 (Konami Wai Wai Racing Advance)
      //   exec 112,635/280,896 cy/frame (60% idle) — CPU 70% of budget
      "AKWJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
      0x8000422,                   /* idle_loop_target_pc  */
      0,                           /* translation_gate_target_1 */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 112,853/280,896 (60% idle) — CPU 71% of budget
    */

   /* BGJJ  환성신 저스티라이더 - 지구의 전사들 (Genseishin Justirisers - Souchaku! Hoshi no Senshi-tachi)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 113,628/280,896 (60% idle) — CPU 71% of budget
    */

   /* AMKE  마리오 카트 어드밴스 (Mario Kart - Super Circuit)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 114,147/280,896 (59% idle) — CPU 71% of budget
    */

   {
      // 유희왕! - 듀얼 몬스터즈 인터네셔널2
      //   exec 115,052/280,896 cy/frame (59% idle) — CPU 72% of budget
      "BYIJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 115,134/280,896 (59% idle) — CPU 72% of budget
    */

   /* AHQJ  하로봇츠 로보히어로 배틀링 (Harobots - Robo Hero Battling!!)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 115,740/280,896 (59% idle) — CPU 72% of budget
    */

   /* AVEE  얼티밋 비치 사커 (Ultimate Beach Soccer)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 115,999/280,896 (59% idle) — CPU 72% of budget
    */

   {
      // 파이널 판타지 택틱스 어드밴스 (한글패치)
      //   exec 116,375/280,896 cy/frame (59% idle) — CPU 73% of budget
      "AFXJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 117,238/280,896 (58% idle) — CPU 73% of budget
    */

   {
      // 록맨 제로 4 (한글패치)
      //   exec 119,971/280,896 cy/frame (57% idle) — CPU 75% of budget
      "B4ZJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
...

   {
      ...
      //   exec 120,415/280,896 cy/frame (57% idle) — CPU 75% of budget
      "AGOJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 120,601/280,896 (57% idle) — CPU 75% of budget
    */

   /* AXHJ  단도시 (Dan Doh!! Xi)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 121,017/280,896 (57% idle) — CPU 76% of budget
    */

   /* BHAJ  꽃놀이 어드밴스 (Hanabi Hyakkei Advance)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 121,361/280,896 (57% idle) — CPU 76% of budget
    */

   {
      // 디즈니 스포츠 - 풋볼 (Disney Sports - Football)
      //   exec 121,504/280,896 cy/frame (57% idle) — CPU 76% of budget
      "A6DP",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 121,644/280,896 (57% idle) — CPU 76% of budget
    */

   {
      // 디즈니 스포츠 - 축구 (Disney Sports - Soccer)
      //   exec 122,019/280,896 cy/frame (57% idle) — CPU 76% of budget
      "A6DE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
      0x8000424,                   /* idle_loop_target_pc  */
      0,                           /* translation_gate_target_1 */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 122,514/280,896 (56% idle) — CPU 77% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 123,467/280,896 (56% idle) — CPU 77% of budget
    */

   {
      // 록맨 제로 2 (Megaman Zero 2)
      //   exec 124,222/280,896 cy/frame (56% idle) — CPU 78% of budget
      "A62J",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 124,500/280,896 (56% idle) — CPU 78% of budget
    */

   /* AZUE  스트리트 파이터 알파 3 (Street Fighter Alpha 3)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 124,508/280,896 (56% idle) — CPU 78% of budget
    */

   /* A88J  마리오와 루이지RPG
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 124,915/280,896 (56% idle) — CPU 78% of budget
    */

   {
      // 철권 어드밴스
      //   exec 126,045/280,896 cy/frame (55% idle) — CPU 79% of budget
      "ATKJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 126,579/280,896 (55% idle) — CPU 79% of budget
    */

   /* AGTJ  전일본GT선수권 (Zen-Nihon GT Senshuken)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 127,148/280,896 (55% idle) — CPU 79% of budget
    */

   /* AAUJ  진 여신전생
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 128,628/280,896 (54% idle) — CPU 80% of budget
    */

   {
      // 슈퍼로봇대전 OG (Super Robot Taisen - Original Generation)
      //   exec 128,676/280,896 cy/frame (54% idle) — CPU 80% of budget
      "ASRJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 128,680/280,896 (54% idle) — CPU 80% of budget
    */

   /* BFFJ  파이널판타지I&II어드밴스0.5
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 128,792/280,896 (54% idle) — CPU 80% of budget
    */

   {
      // 메탈슬러그 어드밴스 (Metal Slug Advance) (2)
      //   exec 129,199/280,896 cy/frame (54% idle) — CPU 81% of budget
      "BSMJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
      0x8000298,                   /* idle_loop_target_pc  */
      0,                           /* translation_gate_target_1 */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 129,370/280,896 (54% idle) — CPU 81% of budget
    */

   /* BR3E  알타입 III (R-Type III - The Third Lightning)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 129,801/280,896 (54% idle) — CPU 81% of budget
    */

   /* BREJ  리비에라 - 약속의 땅 리비에라
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 129,812/280,896 (54% idle) — CPU 81% of budget
    */

   /* AK7J  바람의 크로노아 히어로즈 - 전설의 스타메달
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 130,248/280,896 (54% idle) — CPU 81% of budget
    */

   /* AFUJ  요괴도 (Youkaidou)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 131,130/280,896 (53% idle) — CPU 82% of budget
    */
...

   {
      ...
      //   exec 131,969/280,896 cy/frame (53% idle) — CPU 82% of budget
      "AEMJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
   ...
   {
      // 에고 매니아 (Eggomania)
      //   exec 132,729/280,896 cy/frame (53% idle) — CPU 83% of budget
      "AEMP",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
...

   {
      ...
      //   exec 135,133/280,896 cy/frame (52% idle) — CPU 84% of budget
      "B3MJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 135,258/280,896 (52% idle) — CPU 85% of budget
    */

   /* BN2J  나루토 - 최강닌자 대결집!2
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 135,327/280,896 (52% idle) — CPU 85% of budget
    */

   /* AZAJ  아즈망가대왕 어드밴스
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 135,606/280,896 (52% idle) — CPU 85% of budget
    */

   /* BUVJ  우주의 스텔비아 (Uchuu no Stellvia)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 135,923/280,896 (52% idle) — CPU 85% of budget
    */

   /* B8KJ  별의 카비 - 거울속의 대미궁
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 136,289/280,896 (51% idle) — CPU 85% of budget
    */

   /* B8KE  별의 커비 - 거울의 대미궁 (Kirby &amp; The Amazing Mirror)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 136,386/280,896 (51% idle) — CPU 85% of budget
    */

   {
      // 철권 어드밴스 (Tekken Advance)
      //   exec 136,502/280,896 cy/frame (51% idle) — CPU 85% of budget
      "ATKE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
      0x800074a,                   /* idle_loop_target_pc  */
      0,                           /* translation_gate_target_1 */
   ...
   {
      // 슈퍼마리오월드 (한글패치)
      //   exec 137,516/280,896 cy/frame (51% idle) — CPU 86% of budget
      "AA2C",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 138,188/280,896 (51% idle) — CPU 86% of budget
    */

   {
      // 슈퍼마리오 브라더스3 (Super Mario Advance 4 - Super Mario 3 + Mario Brothers)
      //   exec 139,692/280,896 cy/frame (50% idle) — CPU 87% of budget
      "AX4J",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 139,956/280,896 (50% idle) — CPU 87% of budget
    */

   {
      // 메이드 인 와리오 (한글패치)
      //   exec 140,911/280,896 cy/frame (50% idle) — CPU 88% of budget
      "AZWJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 142,231/280,896 (49% idle) — CPU 89% of budget
    */

   {
      // 록맨 제로 3 (한글패치)
      //   exec 142,669/280,896 cy/frame (49% idle) — CPU 89% of budget
      "BZ3J",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 145,220/280,896 (48% idle) — CPU 91% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 145,354/280,896 (48% idle) — CPU 91% of budget
    */

   /* BGAJ  SD 건담 G 제네레이션 어드밴스 (SD Gundam G Generation Advance)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 146,285/280,896 (48% idle) — CPU 91% of budget
    */
    ...
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 153,069/280,896 (46% idle) — CPU 96% of budget
    */

   /* APWE  파워레인저 - 와일드포스 (Power Rangers - Wild Force)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 154,376/280,896 (45% idle) — CPU 96% of budget
    */

   /* SV3D  바룸3d레이싱 (Varooom 3D)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 156,150/280,896 (44% idle) — CPU 98% of budget
    */

   /* B36J  진삼국무쌍 어드밴스 (Shin Sangoku Musou Advance)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 156,407/280,896 (44% idle) — CPU 98% of budget
    */

   /* BJHE  저스티스 리그 히어로즈 플래시 (Justice League Heroes - The Flash)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 156,444/280,896 (44% idle) — CPU 98% of budget
    */

   /* AORJ  오리엔탈 블루 - 푸른천외 (Oriental Blue - Ao no Tengai)
    * No busy-wait loop — it waits via the BIOS (SWI IntrWait/Halt), which gpSP
    * already fast-forwards (cpu.cc:1499). No entry needed; not slow.
    * exec 157,304/280,896 (44% idle) — CPU 98% of budget
    */
...

   {
      ...
      //   exec 173,660/280,896 cy/frame (38% idle) — CPU 109% of budget
      "A3AJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
...

   {
      ...
      //   exec 178,575/280,896 cy/frame (36% idle) — CPU 112% of budget
      "BDQE",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
...

   {
      ...
      //   exec 178,881/280,896 cy/frame (36% idle) — CPU 112% of budget
      "BDQJ",                      /* gamepak_code         */
      0,                           /* flags (gpSP auto-detects the save type) */
      0x80003a8,                   /* idle_loop_target_pc  */
      0,                           /* translation_gate_target_1 */

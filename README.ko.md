...
- **공식 지원 21개 시스템 전부** — 업스트림
  [sylverb 펌웨어](https://github.com/sylverb/game-and-watch-retro-go-sd)가 등록하는
  기종 전체: NES, 게임보이 / GB 컬러, 게임기어, 마스터시스템, 제네시스,
  SG-1000, PC 엔진, **PC 엔진 CD**, 콜레코비전, MSX, 아타리 2600 / 7800 / **링스**,
  암스트라드 CPC, 와타라, 다마고치, 포켓몬 미니, Game & Watch, 홈브루, PICO-8.
...
홈브루 앱(젤다3, 슈퍼 마리오 월드, 셀레스테 …)은 **펌웨어에 함께 빌드**돼 있어서,
"설치"란 결국 데이터 파일을 `/roms/homebrew/`에 넣어 두는 일입니다. 파일은 두 종류뿐이고,
이 둘을 섞는 게 실행이 깨지는 원인입니다.

- **플래시한 펌웨어에서 꺼내는 것** — `.bin` 실행 템플릿과 그 짝(`zelda3.ro`, `sm.xip`).
  해당 빌드와 한 쌍이라, 다른 릴리즈에서 가져온 파일은 주소가 달라 실행 즉시 죽습니다.
  반드시 **기기에 실제로 플래시한 릴리즈**의 파일을 쓰세요.
- **본인 원본 롬으로 만드는 것** — 애셋 파일(슈퍼 메트로이드는 롬 자체). 저작권 있는
  게임 데이터라 펌웨어도, 이 앱도 제공하지 않습니다.
...
라이브러리의 해당 카드에서 **데이터 파일 추가/교체**로 올리거나, **추가파일** 탭에
`roms/homebrew/…` 경로로 올리면 됩니다. 정보(INFO) 탭에 같은 표가 있고 경로는 클릭 복사됩니다.
알아두면 좋은 것:

- **`.bin` 이름은 바꾸지 마세요.** 펌웨어가 파일명으로 앱을 고르기 때문에, 이름을 바꾸면
  로드는 되지만 아무 앱과도 매칭되지 않습니다. (라이브러리에서 이름 변경을 막아 둡니다.)
- **없으면 대부분 화면에 알려주는데, 하나만 예외입니다.** `.dat`이 없으면 화면에
  `Missing …_assets.dat`으로 멈추지만, `zelda3.ro`가 없으면 아무 말 없이 죽습니다.
- **슈퍼 메트로이드는 플래시 증설이 필요합니다.** 롬을 외장 플래시에 캐시하는데
  3 MB는 순정 1 MB에 들어가지 않습니다.
- 다른 롬(또는 다른 버전 도구)으로 만든 `.dat`은 `Mismatching …_assets.dat`으로 거부됩니다.
